    ...
    return new Promise((res, rej) => {
      ...
      this.ws.onopen = async () => {
        this.ws.onmessage = (ev) => {
          ...
          if (m.method === 'Runtime.exceptionThrown') { var d = m.params.exceptionDetails; this.consoleErrors = (this.consoleErrors || []).concat([d.text + ' ' + ((d.exception || {}).description || '') + ' @' + (d.url || '') + ':' + (d.lineNumber || '')]); }
        };
